...
function parseComp(document) {
    ...
    while (i < declEnd) {
        ...
        if (optM) {
            ...
            if (optName === 'userspace' && optVal !== 'no')
                isUserspace = true;
            if (optName === 'singleton' && optVal !== 'no')
                hasSingleton = true;
            declarations.push({
...
function getDiagnostics(document) {
    ...
    if (parsed.separatorLine < 0) {
        ...
        diags.push(err(new vscode.Range(lastLine, 0, lastLine, lines[lastLine]?.length ?? 0), 'Missing ";;" separator — declarations must be followed by ";;" then C code'));
    }
    ...
    for (const decl of parsed.declarations) {
        if (decl.kind === 'pin' || decl.kind === 'param' || decl.kind === 'function' || decl.kind === 'variable') {
            if (seen.has(decl.name)) {
                diags.push(err(decl.range, `Duplicate identifier "${decl.name}" (first declared at line ${seen.get(decl.name) + 1})`));
            }
            else {
    ...
    for (const decl of parsed.declarations) {
        if ((decl.kind === 'pin' || decl.kind === 'param') &&
            ...
            diags.push(hint(decl.range, `"${decl.type}" is deprecated; prefer "${decl.type === 's32' ? 'signed' : 'unsigned'}"`));
        }
    }
    ...
    for (let lineIdx = 0; lineIdx < declEnd; lineIdx++) {
        ...
        if (kwMatch && !VALID_DECL_KEYWORDS.has(kwMatch[1])) {
            ...
            diags.push(warn(new vscode.Range(lineIdx, col, lineIdx, col + kwMatch[1].length), `Unknown declaration keyword "${kwMatch[1]}"`));
        }
        // Pin with 'in' direction named 'in' (C keyword collision)
        if (/^\s*pin\s+in\s+\S+\s+in\b/.test(raw)) {
            diags.push(warn(new vscode.Range(lineIdx, 0, lineIdx, raw.length), 'Pin named "in" collides with C keyword; consider appending "_" (e.g. "in_")'));
        }
        // param with rw direction but no '=' default (not an error but helpful)
        if (/^\s*param\s+rw\s/.test(raw) && !raw.includes('=') && raw.includes(';')) {
            diags.push(hint(new vscode.Range(lineIdx, 0, lineIdx, raw.length), 'Read-write parameter has no default value (will default to 0/FALSE)'));
        }
    }
    return diags;
}
...
function makeSnippet(item, kind) {
    ...
    if (inC) {
        ...
        for (const decl of parsed.declarations) {
            if (decl.kind === 'pin' || decl.kind === 'param' || decl.kind === 'variable') {
                ...
                if (decl.doc)
                    ci.documentation = new vscode.MarkdownString(decl.doc);
                items.push(ci);
            }
...
function provideDefinition(document, position) {
    ...
    if (parsed.separatorLine < 0 || position.line <= parsed.separatorLine)
        return null;
    const wordRange = document.getWordRangeAtPosition(position, /[A-Za-z_][A-Za-z0-9_]*/);
    if (!wordRange)
        return null;
    const word = document.getText(wordRange);
    const decl = parsed.declarations.find(d => d.name === word ||
...
function activate(context) {
    ...
    // Completion
    context.subscriptions.push(vscode.languages.registerCompletionItemProvider({ language: 'linuxcnc-comp' }, {
        provideCompletionItems(doc, pos) {
            return provideCompletions(doc, pos);
        }
    }, ' ', '\t'));
    // Hover
    context.subscriptions.push(vscode.languages.registerHoverProvider({ language: 'linuxcnc-comp' }, {
        provideHover(doc, pos) {
            return provideHover(doc, pos);
        }
    }));
    // Go-to definition
    context.subscriptions.push(vscode.languages.registerDefinitionProvider({ language: 'linuxcnc-comp' }, {
    ...
    // Formatter
    context.subscriptions.push(vscode.languages.registerDocumentFormattingEditProvider({ language: 'linuxcnc-comp' }, {
...
function alignPinParamBlock(lines, kw) {
    ...
    if (parsed.some(p => p === null))
        return lines.map(l => normalizeDeclSpacing(l));
    const ps = parsed;
...
function alignFunctionBlock(lines) {
    ...
    return ps.map(p => {
        ...
        if (p.qualifier)
            line += ' ' + (p.doc ? p.qualifier.padEnd(qualW) : p.qualifier);
        if (p.doc)
            line += ' ' + p.doc;
        line += ';';
        if (p.comment)
...
function formatDeclSection(lines) {
    ...
    while (i < lines.length) {
        ...
        if (kw === 'pin' || kw === 'param' || kw === 'function' || kw === 'variable') {
            ...
            let aligned;
            if (kw === 'pin' || kw === 'param')
                aligned = alignPinParamBlock(block, kw);
            else if (kw === 'function')
                aligned = alignFunctionBlock(block);
            else
                aligned = alignVariableBlock(block);
            result.push(...aligned);
            i = j;
        }
        else {
